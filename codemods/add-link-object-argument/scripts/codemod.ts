...
function isDdTraceMemberCall(node: SgNode<TSX> | null | undefined, method: string, bindings: Set<string>): boolean {
  if (!isMemberCall(node, method)) return false;
  if (!node) return false;
  const fn = node.field("function");
...
function isDdTraceSpanFactory(node: SgNode<TSX> | null | undefined, bindings: Set<string>): boolean {
  if (!node || node.kind() !== "call_expression") return false;
  if (isDdTraceMemberCall(node, "startSpan", bindings)) return true;

  if (!isMemberCall(node, "active")) return false;
  const fn = node.field("function");
...
const transform: Transform<TSX> = async (root) => {
  ...
  for (const call of calls) {
    if (!isAddLinkCall(call)) continue;
    ...
    if (!contextArg || !attributesArg) continue;
