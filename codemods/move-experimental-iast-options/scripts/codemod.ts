...
function pairTextWithLeadingComments(pair: SgNode<TSX>, source: string, toIndent: string, includeFirstIndent: boolean): string {
  const start = leadingCommentStart(pair, source);
  const fromIndent = start === pair.range().start.index
